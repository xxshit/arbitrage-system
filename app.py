...
def thought_lark_ake_wall_message(analysis, direction):
    ...
    if direction == "ake_wall_test":
        ...
        judgement = "判断：这是你的当前盘面假设，不作为固定规则。AKE 插到 0.002 上方后马上回落，可能是诱空，也可能是真失败；我先不直接判死。接下来验证重点是：能否重新站上 0.0020、持仓是否不塌、CVD 是否没有连续恶化、下跌是否没有放量。"
        key_zone = "验证：重新站上 0.0020，假设增强；进入 0.0020-0.0022，继续看上涨转强；站上 0.0022，升级为强势逼空。证伪：跌破 0.00190 且价格/CVD/持仓同步转弱，降级为墙下失败。"
