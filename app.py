...
SUDDEN_PUMP_PRE_IGNITION_THRESHOLD = 10.0
SUDDEN_PUMP_PRE_IGNITION_MAX_AGE_SECONDS = 3 * 60
SUDDEN_PUMP_PRE_IGNITION_MIN_VOLUME_PACE = 3.0
SUDDEN_PUMP_FAST_SCAN_SECONDS = 5.0
SUDDEN_PUMP_FAST_SCAN_HOLD_SECONDS = 30.0
TREND_WINDOWS = {
    "change_5m": 5 * 60,
    "change_15m": 15 * 60,
    "change_30m": 30 * 60,
...
SUDDEN_PUMP_FAST_SCAN_UNTIL = 0.0
RUNTIME_RULE_DEFAULTS = [
    ...
    {"rule_key": "sudden_pump_scan", "category": "趋势筛选", "label": "全市场 5MIN 急涨扫描", "schedule_type": "interval", "value": "10", "unit": "秒", "min_value": 5, "max_value": 60, "description": "轻量读取本地5分钟价格桶；候选出现后30秒内临时加快到5秒，并向 Binance 复核短周期点火。"},
    {"rule_key": "sudden_pump_threshold", "category": "趋势筛选", "label": "5MIN 正式急涨阈值", "schedule_type": "threshold", "value": "15", "unit": "%", "min_value": 5, "max_value": 50, "description": "10%可在前3分钟且量速/CVD确认后预警；达到本阈值进入正式急涨提醒。盘中均需连续确认两次。"},
...
def sudden_pump_bullish_confirmation(item):
    ...
    return (
        item.get("stage_key") in {"acceleration", "extreme_15m"}
...
def sudden_pump_pre_ignition_stage(change_5m, elapsed_seconds, volume_pace_ratio, cvd):
    """Return an early warning only for a fast, buy-led move in the first three candle minutes."""
    if not (
        float(change_5m or 0) >= SUDDEN_PUMP_PRE_IGNITION_THRESHOLD
        and float(elapsed_seconds or 0) <= SUDDEN_PUMP_PRE_IGNITION_MAX_AGE_SECONDS
        and float(volume_pace_ratio or 0) >= SUDDEN_PUMP_PRE_IGNITION_MIN_VOLUME_PACE
        and float(cvd or 0) > 0
    ):
        return None
    return {
        ...
        "stage_rank": 1,
        "stage_reason": (
            f"当前5MIN尚未达到正式急涨线，但前{float(elapsed_seconds):.0f}秒已上涨"
            f"{float(change_5m):.2f}%，折算量能速度约为近期{float(volume_pace_ratio):.2f}倍。"
        ),
    }


def sudden_pump_scan_delay(configured_seconds, fast_until=None, now=None):
    fast_until = SUDDEN_PUMP_FAST_SCAN_UNTIL if fast_until is None else float(fast_until)
    now = time.monotonic() if now is None else float(now)
    configured_seconds = float(configured_seconds)
    return min(configured_seconds, SUDDEN_PUMP_FAST_SCAN_SECONDS) if now < fast_until else configured_seconds


def sudden_pump_confirmation_ready(
    symbol, bucket_at, closed_trigger, observed_at=None, observed_epoch=None, stage_key=None
):
    ...
    key = (symbol, int(bucket_at), str(stage_key or "default"))
    state = SUDDEN_PUMP_CONFIRMATIONS.get(key)
    if not state:
        SUDDEN_PUMP_CONFIRMATIONS[key] = {
...
def fetch_sudden_pump_context(symbol, threshold=15.0, now_epoch=None):
    """Verify a local candidate and distinguish early pre-ignition from the formal 15% alert."""
    raw_symbol = symbol.replace("/", "")
    now_epoch = time.time() if now_epoch is None else float(now_epoch)
    pre_threshold = min(SUDDEN_PUMP_PRE_IGNITION_THRESHOLD, float(threshold))
    try:
        klines = get_json("https://fapi.binance.com/fapi/v1/klines?" + urlencode({
        ...
        elapsed_seconds = 300.0 if closed_trigger else max(1.0, now_epoch - float(current[0]) / 1000.0)
        volume_progress = max(30.0, min(300.0, elapsed_seconds)) / 300.0
        volume_pace_ratio = volume_ratio / volume_progress if volume_ratio is not None else None

        if change_5m >= threshold:
            stage = classify_sudden_pump_stage(change_5m, change_15m, hot_bar_count)
        else:
            stage = sudden_pump_pre_ignition_stage(
                change_5m, elapsed_seconds, volume_pace_ratio, cvd
            )
            if not stage:
                return None
        ...
        oi_change = None
        ratio_change = None
        if stage["stage_key"] != "pre_ignition":
            oi_rows = get_json("https://fapi.binance.com/futures/data/openInterestHist?" + urlencode({
        ...
        item = {
            ...
            "candle_elapsed_seconds": elapsed_seconds,
            "formal_threshold": float(threshold),
...
def sudden_pump_rough_candidates(snapshot, threshold=15.0, now_bucket=None):
    ...
    if not groups:
        return []
    candidate_threshold = min(SUDDEN_PUMP_PRE_IGNITION_THRESHOLD, float(threshold)) * 0.9
    ...
    for group in groups:
        ...
        if (active_change is not None and active_change >= candidate_threshold) or (closed_change is not None and closed_change >= threshold * 0.9):
    ...
    row = PushSignalValidation(
        ...
        snapshot_json=json.dumps({
            ...
            for key in (
                ...
                "volume_ratio", "volume_pace_ratio", "candle_elapsed_seconds", "formal_threshold", "closed_trigger",
                "trigger_bucket", "bullish_confirmed", "first_detected_epoch", "confirmed_epoch",
                "lark_sent_epoch", "detection_to_confirmation_seconds", "confirmation_to_lark_seconds",
            )
        }, ensure_ascii=False),
    )
    db.session.add(row)
...
def sudden_pump_push_section(item, confirmation, rendered_at=None):
    ...
    elif item.get("stage_key") == "pre_ignition":
        heading = lark_dot_label("↑ 极速预点火 / 提前观察", "orange")
        title = f"**{symbol}　5MIN急涨前置预警**"
        judgement = (
            f"提前判断：{item['stage_reason']}当前尚未达到正式"
            f"{float(item.get('formal_threshold') or 15):.0f}%急涨确认线。"
        )
        risk = "风险提示：预点火只代表上涨速度和主动买量异常，不等于看涨确认；跌回起涨区或CVD转负即失效。"
    else:
...
def send_sudden_pump_push(items):
    ...
    stage_ranks = {"pre_ignition": 1, "ignition": 2, "acceleration": 3, "extreme_15m": 4}
    for item in items:
    ...
    for item, state_keys, validation_key in reserved:
        item["lark_sent_epoch"] = trigger_epoch
        first_detected_epoch = int(item.get("first_detected_epoch") or trigger_epoch)
        confirmed_epoch = int(item.get("confirmed_epoch") or trigger_epoch)
        item["detection_to_confirmation_seconds"] = max(0, confirmed_epoch - first_detected_epoch)
        item["confirmation_to_lark_seconds"] = max(0, trigger_epoch - confirmed_epoch)
...
def scan_intraday_sudden_pumps():
    ...
    global SUDDEN_PUMP_FAST_SCAN_UNTIL
    ...
    try:
        ...
        if not symbols:
            return 0
        SUDDEN_PUMP_FAST_SCAN_UNTIL = max(
            SUDDEN_PUMP_FAST_SCAN_UNTIL,
            time.monotonic() + SUDDEN_PUMP_FAST_SCAN_HOLD_SECONDS,
        )
        verified = []
        with ThreadPoolExecutor(max_workers=min(4, len(symbols))) as executor:
            ...
            for future in as_completed(futures):
                ...
                if sudden_pump_confirmation_ready(
                    ...
                    observed_at=observed_at, observed_epoch=observed_epoch,
                    stage_key=item["stage_key"],
                ):
                    confirmation = SUDDEN_PUMP_CONFIRMATIONS.get(
                        (item["symbol"], int(item["trigger_bucket"]), item["stage_key"]), {}
                    )
                    item["first_detected_epoch"] = int(
                        confirmation.get("first_seen_epoch", observed_epoch)
                    )
                    item["confirmed_epoch"] = int(observed_epoch)
...
def thought_coti_item(coti):
    ...
    return {
        ...
        "assistant_mistakes": [
            ...
            "进一步修正：我随后又把负基差、负资费过快解释为‘空头拥挤’，仍然属于单一叙事。主力也可能先拉现货、压住合约制造贴水与负资费，用它吸引多头、推动现货，或为换手准备对手盘。以后只能把空头拥挤列为竞争假设之一。",
...
def background_intraday_sudden_pump_scan():
    ...
    while True:
        ...
        configured_delay = runtime_interval("sudden_pump_scan", 10)
        time.sleep(sudden_pump_scan_delay(configured_delay))
