...
def daily_report_thoughts():
    ...
    return jsonify({
        ...
        "items": [{
            ...
            "thesis_win_rate": {"wins": 1, "losses": 0, "pending": 1, "rate": 100.0, "note": "样本很少，只作为当前复盘统计，不当成稳定胜率。"},
            "my_thesis": "你的主线思路：犄型走势必须是持仓上涨，同时多空人数比呈对称下跌，符合主力做多、散户做空；如果 CVD 也上涨，更有力说明主动买入资金在推进，主力可能仍在布置多单。AKE 入场点 0.00085，后续止盈止损必须围绕这个真实成本来定。",
            "assistant_thesis": "我的验证思路：我会按你的主线去盯盘，但不会只因为价格上涨就推送；必须看到持仓继续扩张、多空人数比继续下行、CVD 继续为正，并且价格没有明显滞涨。若人数比回升、CVD 背离或持仓假扩张，我会把结论从主力吸筹延续降级为高位换手/派发风险。",
            "challenge_points": [
            ...
            "stop_loss": [
                f"{ake['entry'] * 1.08:.8f}-{ake['entry'] * 1.11:.8f}：保护止损区，基于你的 0.00085 入场价保住 8%-11% 浮盈；跌破说明短线突破动能减弱。",
                f"{ake['entry'] * 0.99:.8f}-{ake['entry']:.8f}：结构止损区，跌回入场价附近且无法快速收回，说明这次拉升逻辑失败，不应继续硬扛。",
                f"{ake['support']:.8f} 附近：强制风控位，跌破近端支撑且 OI/CVD 同步回落，应按趋势失效处理。",
            ],
            "review_notes": [
